...
def redact_text(text, selected_entities, masking_style="Tags"):
    ...
    """
    redacted_text = text
    detected_items = [] # Stores: (Text, Label, Start, End)

    # --- 1. Regex Patterns (Strict Mandatory List) ---
    patterns = {
        "URL": r'(https?://\S+|www\.\S+)',
    ...
    for label, pattern in patterns.items():
        if label in selected_entities:
            for match in re.finditer(pattern, redacted_text):
                ...
                # Store details BEFORE replacing (approximate logic for demo)
    ...
    if "ner" in nlp.pipe_names:
        ...
        for text_chunk, label, start, end in ai_replacements:
            ...
            detected_items.append({
                "Entity": label,
                "Text": text_chunk,
                "Start": "AI-Calc", # Dynamic in modified text
                "End": "AI-Calc"
            })
